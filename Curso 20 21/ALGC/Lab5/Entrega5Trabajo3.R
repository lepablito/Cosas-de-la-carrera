...
algoritmoaestrella<-function(matrizoriginal,rowsal,colsal,rowdes,coldes,semillapesos){
  ...
  matrizdist<-matrix(nrow=tammat[1], ncol = tammat[2])
  ...
  while(TRUE){
    ...
    if(is.na(explorado[pos[1]+1,pos[2]]) & matrizoriginal[pos[1]+1,pos[2]]!=-10){
      disttent<-matrizalg[pos[1],pos[2]]+matrizoriginal[pos[1]+1,pos[2]]+(abs(rowdes-(pos[1]+1))+abs(coldes-pos[2]))
      if(!is.na(matrizalg[pos[1]+1,pos[2]])){
        if (disttent<matrizalg[pos[1]+1,pos[2]]){
          matrizalg[pos[1]+1,pos[2]]<-disttent
          matrizdist[pos[1]+1,pos[2]]<-disttent-(abs(rowdes-(pos[1]+1))+abs(coldes-pos[2]))
        }
      }else{
        matrizalg[pos[1]+1,pos[2]]<-disttent
        matrizdist[pos[1]+1,pos[2]]<-disttent-(abs(rowdes-(pos[1]+1))+abs(coldes-pos[2]))
    ...
    if(is.na(explorado[pos[1],pos[2]+1]) & matrizoriginal[pos[1],pos[2]+1]!=-10){
      disttent<-matrizalg[pos[1],pos[2]]+matrizoriginal[pos[1],pos[2]+1]+(abs(rowdes-pos[1])+abs(coldes-(pos[2]+1)))
      if(!is.na(matrizalg[pos[1],pos[2]+1])){
        if (disttent<matrizalg[pos[1],pos[2]+1]){
          matrizalg[pos[1],pos[2]+1]<-disttent
          matrizdist[pos[1],pos[2]+1]<-disttent-(abs(rowdes-pos[1])+abs(coldes-(pos[2]+1)))
        }
      }else{
        matrizalg[pos[1],pos[2]+1]<-disttent
        matrizdist[pos[1],pos[2]+1]<-disttent-(abs(rowdes-pos[1])+abs(coldes-(pos[2]+1)))
...
#####################################################
lista<-generaLaberinto(50,1,1234,4321)
...
####################################################
t<-proc.time()
dijkstrafrontera<-algoritmoaestrella(lista$matriz,lista$rowsal,lista$colsal,lista$rowdes,lista$coldes,999)
tiempoaestrella<-proc.time()-t
